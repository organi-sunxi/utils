...
static void SetQtEnv()
{
	...
	if(cal_wh(&mmW, &mmH)==0){
		sprintf(buffer, "%d", mmW);
		setenv("QT_QPA_EGLFS_PHYSICAL_WIDTH", buffer, 1);
		sprintf(buffer, "%d", mmH);
		setenv("QT_QPA_EGLFS_PHYSICAL_HEIGHT", buffer, 1);
	}
