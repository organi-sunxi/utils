...
#define format_to_name(f, n)	((f) == n) ? #n :

#define format_name(f)	format_to_name(f, V4L2_PIX_FMT_YUV422P)	\
				format_to_name(f, V4L2_PIX_FMT_YUYV)	\
				format_to_name(f, V4L2_PIX_FMT_YVYU)	\
				format_to_name(f, V4L2_PIX_FMT_UYVY)	\
				format_to_name(f, V4L2_PIX_FMT_VYUY)	\
				format_to_name(f, V4L2_PIX_FMT_YUV420)	\
				format_to_name(f, V4L2_PIX_FMT_NV16)	\
				format_to_name(f, V4L2_PIX_FMT_NV12)	\
				format_to_name(f, V4L2_PIX_FMT_HM12)	\
				"unknown"
...
#else
		pvdev->buff_info[i].length = pvdev->offset[0]*2;
		pvdev->buff_info[i].start = malloc(pvdev->buff_info[i].length+4096);
		buf.length = pvdev->buff_info[i].length;
		buf.m.userptr = (unsigned long)pvdev->buff_info[i].start;
#endif
...
static void write_file(char *p, struct video_dev *pvdev)
{
	int y,w=pvdev->cap_width, h=pvdev->cap_height;

	//write Y data
	for(y=0; y<h; y+=2){
		fwrite(p, w, 1, capfile);
		p+=(w<<1);
	}
...
int video_stop(void)
{
	...
	if(capfile){
		fseek(capfile, 0L, SEEK_SET);
		fwrite(&video_hd, sizeof(video_hd), 1, capfile);
	}
